...
import com.FlappyScrappy_LTF.Common.PlayerInfo;

public class ScoreLayer extends CCLayer {

	private final float BACK_POS_X     = (50 * Common.kXForIPhone);
	private final float BACK_POS_Y    =  (235 * Common.kYForIPhone);

	//private final float RANK_NUM_X         = (150 * Common.kXForIPhone);
	//private final float RANK_NAME_X        = (68 * Common.kXForIPhone);
	//private final float RANK_SCORE_X       = (220 * Common.kXForIPhone);

	private final float RANK_NUM_X         = (Common.SCREEN_WIDTH / 2 - 150);
	private final float RANK_SCORE_X       = (Common.SCREEN_WIDTH / 2 + 55);
	
	private CCSprite topscores;
	private CCMenuItemImage returnButton;
	private CCSprite background;
	private CCSprite headerbackground;
	...
	private void init(){
        ...
        topscores.setScaleX(.5f * Common.kXForIPhone);
        topscores.setScaleY(.5f * Common.kYForIPhone);
        topscores.setPosition(Common.SCREEN_WIDTH / 2, 235 * Common.kYForIPhone);
        addChild(topscores, 4);

        headerbackground = CCSprite.sprite("topscores_header_back.png");
        headerbackground.setScaleX(.5f * Common.kXForIPhone);
        headerbackground.setScaleY(.5f * Common.kYForIPhone);
        headerbackground.setPosition(Common.SCREEN_WIDTH / 2, 272 * Common.kYForIPhone);//Common.SCREEN_HEIGHT - topscores.getContentSize().height / 2 * topscores.getScaleY());
        addChild(headerbackground, 3);
        
        returnButton = CCMenuItemImage.item("back.png", "back.png", this, "onBack");
        returnButton.setScale(.15f * Common.kXForIPhone);
        ...
        float itemPosY = 200 * Common.kYForIPhone;
