...
public class MoveLayer extends CCLayer {
	...
	public final float FIRST_BODY_Y =   (0 * Common.kYForIPhone);
	        ...
	        switch (i) {
	            case 0:
	            {
	                ...
	                for (int index = 0; index<Common.IMG_BG1_BACK1.length; index ++) {
	                    ...
	                    body.setScaleX(.5F * Common.kXForIPhone);
	                    body.setScaleY(.5F * Common.kYForIPhone);
	...
	public void generateBird()
	{
	    ...
	    if (randFoodType == 0) {
	        bird = Bird.init(randBird, Common.CoinType.FAST);
	    }
	    else
	    {
	        bird = Bird.init(randBird, Common.CoinType.FAST);
	...
	private void initFoods()
	{
	    ...
	    for (int i = 0; i<5; i ++) {
	        if (foods.size()% randRainbow == 0) { // ???
	            food = Coin.init(Common.CoinType.GOLD_COIN);
	        }
	        else if (foods.size() % randBomb == 0) // ???
	        {
	            food = Coin.init(Common.CoinType.SKULL);
	        }
	        else 
	        {
	            food = Coin.init(Common.CoinType.GOLD_COIN);
